...
export default function CascadeKPITracker() {
  ...
  return (
    <div className="p-6 bg-gray-950 min-h-screen text-gray-100">
      ...
      {/* 7. Supporting Signal: PXC (trend-tracking, no status badge) */}
      {/* --- Replace with live data from the 4 PXC queries in references/cascade-kpi-mysql.md --- */}
      {(() => {
        const PXC_INSTANCES = 6740;  // uniqExact(pillar_db_instance_id) WHERE product_family = 'pxc', last 365d
        const PXC_CLUSTERS  = 2954;  // uniqExact(db_replication_id) via ARRAY JOIN metrics, last 365d
        const pxcMonthly = [
          // { month: "Apr 25", instances: 1136 }, ... { month: "Apr 26", instances: 1539 }
        ];
        const pxcVersions = [
          // { name: "8.0.41-32.1", instances: 1188 }, ...
        ];
        const pxcVersionColor = (v) =>
          v.startsWith("8.4") ? "#4CAF50" : v.startsWith("8.0") ? "#FF6B35" : "#6b7280";

        return (
          <div className="bg-gray-900 rounded-xl border border-gray-800 p-5 mb-4">
            <h2 className="text-lg font-semibold text-gray-100 mb-0">Supporting Signal: PXC (Percona XtraDB Cluster)</h2>
            <p className="text-xs text-gray-500 mb-4">One level down from the PS anchor — no Cascade target, trend tracking only</p>

            {/* Two KPI tiles — neutral color, no status tinting */}
            <div className="grid grid-cols-2 gap-3 mb-5">
              <div className="bg-gray-950 rounded-xl border border-gray-800 p-4 text-center">
                <p className="text-xs text-gray-400 font-semibold tracking-wider mb-2">ACTIVE INSTANCES (T12M)</p>
                <p className="text-2xl font-bold text-gray-100">{PXC_INSTANCES.toLocaleString()}</p>
              </div>
              <div className="bg-gray-950 rounded-xl border border-gray-800 p-4 text-center">
                <p className="text-xs text-gray-400 font-semibold tracking-wider mb-2">ACTIVE CLUSTERS (T12M)</p>
                <p className="text-2xl font-bold text-gray-100">{PXC_CLUSTERS.toLocaleString()}</p>
              </div>
            </div>

            {/* Monthly trend — single series, no target/projected lines */}
            <p className="text-xs text-gray-400 font-semibold tracking-wider mb-2">MONTHLY ACTIVE INSTANCES</p>
            <ResponsiveContainer width="100%" height={220}>
              <ComposedChart data={pxcMonthly} margin={{ top: 10, right: 30, left: 20, bottom: 5 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#1f2937" />
                <XAxis dataKey="month" tick={{ fill: "#9ca3af", fontSize: 12 }} />
                <YAxis tick={{ fill: "#9ca3af", fontSize: 12 }} />
                <Tooltip contentStyle={{ backgroundColor: "#1f2937", border: "1px solid #374151", color: "#f3f4f6" }} />
                <Line type="monotone" dataKey="instances" stroke="#4CAF50" strokeWidth={2.5} dot={{ r: 3, fill: "#4CAF50" }} name="PXC Instances" />
              </ComposedChart>
            </ResponsiveContainer>
            <p className="text-xs text-gray-500 mt-2 italic">PXC monthly active is subject to the same Jan 24 2026 pipeline disruption affecting PS. Trailing 12-month cumulative remains reliable.</p>

            {/* Version distribution */}
            <p className="text-xs text-gray-400 font-semibold tracking-wider mt-5 mb-2">VERSION DISTRIBUTION (TOP 10)</p>
            <ResponsiveContainer width="100%" height={240}>
              <BarChart data={pxcVersions} layout="vertical" margin={{ top: 0, right: 10, left: 0, bottom: 0 }}>
                <XAxis type="number" tick={{ fill: "#9ca3af", fontSize: 11 }} />
                <YAxis type="category" dataKey="name" width={110} tick={{ fill: "#d1d5db", fontSize: 11 }} />
                <Tooltip contentStyle={{ backgroundColor: "#1f2937", border: "1px solid #374151", color: "#f3f4f6" }} />
                <Bar dataKey="instances" radius={[0, 4, 4, 0]}>
                  {pxcVersions.map((entry) => (
                    <Cell key={entry.name} fill={pxcVersionColor(entry.name)} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
      ...
      {/* 8. GSM Framework card (PS anchor only) */}
